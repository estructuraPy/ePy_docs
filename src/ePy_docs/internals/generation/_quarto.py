...
from ePy_docs.internals.styling._styler import (
    generate_quarto_config,
    create_quarto_yml
)
from ePy_docs.internals.styling._pages import get_config_value
...
def _should_ignore_file(filename: str, ignore_patterns: List[str]) -> bool:
...
def _cleanup_existing_pdfs(qmd_basename: str, output_dir: str, qmd_dir: str) -> None:
    """Clean up existing PDF files to avoid conflicts."""
    ...
    variants = [
        os.path.join(qmd_dir, f"{qmd_basename.replace(' ', '-')}.pdf"),
        os.path.join(qmd_dir, f"{qmd_basename.replace(' ', '_')}.pdf")
    ]
    for pdf_path in variants:
...
def _find_generated_pdf(qmd_basename: str, qmd_dir: str) -> Optional[str]:
    """Find the PDF file generated by Quarto."""
    possible_names = [
        f"{qmd_basename}.pdf",
    ...
    # Look for any recently created PDF
    ...
    if pdf_files:
        ...
        return os.path.join(qmd_dir, pdf_files_with_time[0][0])
    
    return None

def _move_pdf_to_final_location(generated_pdf: str, final_pdf: str, output_dir: str) -> str:
    """Move generated PDF to final location, handling locks."""
    if not generated_pdf:
        return final_pdf
    
    if os.path.dirname(generated_pdf) != output_dir:
    ...
    if generated_pdf != final_pdf:
        try:
            ...
            return generated_pdf  # Use generated name if can't rename
    else:
...
class QuartoConverter:
    ...
    def _create_qmd_content(self, 
        ...
        from ePy_docs.internals.styling._pages import get_references_config
    ...
    def render_to_pdf(self, qmd_file: str, output_dir: Optional[str] = None) -> str:
        ...
        try:
            _cleanup_existing_pdfs(qmd_basename, output_dir, qmd_dir)
            ...
            generated_pdf = _find_generated_pdf(qmd_basename, qmd_dir)
            
            if not generated_pdf:
                error_msg = f"PDF not generated. QMD: {qmd_path}, Output: {output_dir}"
                if result.stdout:
            ...
            final_pdf = _move_pdf_to_final_location(generated_pdf, final_pdf, output_dir)
            
            if not os.path.exists(final_pdf):
                raise RuntimeError(f"PDF was not generated successfully: {final_pdf}")
            ...
            error_msg = f"Quarto render failed (code {e.returncode})"
            if e.stderr and ("PermissionDenied" in e.stderr or "Acceso denegado" in e.stderr):
                error_msg += "\nPDF file may be open in a reader. Close it and try again."
            if e.stdout:
                error_msg += f"\nStdout: {e.stdout}"
            if e.stderr:
                error_msg += f"\nStderr: {e.stderr}"
    ...
    def render_to_html(self, qmd_file: str, output_dir: Optional[str] = None) -> str:
        ...
        try:
            ...
            try:
                ...
                from ePy_files import save_txt
    ...
    def scan_and_render_directory(self, 
        ...
        if ignore_patterns is None:
            ignore_patterns = get_config_value('components/pages.json', 'ignore_patterns')
        ...
        if recursive:
            for root, dirs, files in os.walk(directory_path):
                qmd_files.extend(
                    ...
                    if f.endswith('.qmd') and not _should_ignore_file(f, ignore_patterns)
                )
        else:
            qmd_files = [
...
def _load_math_config() -> Dict[str, Any]:
...
def load_math_config() -> Dict[str, Any]:
    """Public function to load math configuration - used by other components."""
    return _load_math_config()

def process_mathematical_text(text: str, layout_name: str) -> str:
