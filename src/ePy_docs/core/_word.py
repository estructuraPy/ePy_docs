...
def read_docx_file(file_path: Path, preserve_formatting: bool = True, 
            ...
            if paragraph:
                ...
                if para_text.strip():
                    ...
                    if paragraph.style.name.startswith('Heading'):
                        ...
                        content_parts.append(f"{'#' * level} {para_text}")
                    else:
                        content_parts.append(para_text)
        
        elif element.tag.endswith('tbl') and convert_tables:  # Table
            table = _find_table_by_element(doc, element)
            if table:
                table_md, table_data = _convert_table_to_markdown(table)
                if table_md:
    ...
    if extract_images:
        result['images'] = _extract_images_from_docx(doc, image_output_dir)
...
def _extract_paragraph_with_formatting(paragraph, preserve_formatting: bool = True) -> str:
    ...
    for run in paragraph.runs:
        ...
        try:
            if run._element.rPr and run._element.rPr.strike:
                text = f"~~{text}~~"
        except:
...
def process_word_file(file_path: str, preserve_formatting: bool = True,
    ...
    if extract_images:
        if image_output_dir is None:
            ...
            if output_dir:
                image_output_dir = output_dir / "images" / "extracted_from_word"
            else:
    ...
    try:
        ...
        # Process the content for better integration
        processed_content = process_word_content(content, fix_formatting=True)
        
        # Add spacers to prevent markdown rendering issues
        if writer_instance and writer_instance.content_buffer:
            writer_instance.content_buffer.append('\n\n---\n\n')  # Separator
        
        # Add the processed content
        if writer_instance:
            writer_instance.content_buffer.append(processed_content)
        
        # Handle extracted images - add them as references if needed
        if extract_images and word_data['images']:
            _process_extracted_images(word_data['images'], writer_instance, fix_image_paths)
        
        # Handle tables if they need special processing
        if convert_tables and word_data['tables'] and writer_instance:
            _process_extracted_tables(word_data['tables'], writer_instance)
...
def _process_extracted_images(image_paths: List[str], writer_instance, fix_paths: bool = True):
    """Process extracted images and optionally add them to the document."""
    if not writer_instance:
        return
    
    # For now, we'll add a note about extracted images
    # In the future, this could be enhanced to automatically embed them
    if image_paths:
        writer_instance.content_buffer.append(
            f"\n\n> **Note**: {len(image_paths)} images were extracted from the Word document "
            f"and saved to the images directory.\n\n"
        )


def _process_extracted_tables(tables: List[List[List[str]]], writer_instance):
    """Process extracted table data if needed."""
    # Tables are already converted to markdown in the main content
    # This function is for any additional processing that might be needed
    pass
