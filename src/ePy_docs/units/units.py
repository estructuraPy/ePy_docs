...
def convert_to_default_units(df: pd.DataFrame, column_units: Dict[str, str],
    ...
    units_config = units_data.get('categories', {})
...
# ===== GATEWAY FUNCTIONS FOR UNITS.JSON ACCESS =====
# All access to units.json configuration must go through these functions

def load_units_config(sync_files: bool = False) -> dict:
    """Load units configuration using centralized cache system.
    
    This is the exclusive gateway for accessing units.json configuration.
    No other function should directly access units.json files.
    
    Args:
        sync_files: Whether to sync files before loading
        
    Returns:
        Dictionary with units configuration
    """
    from ePy_docs.core.setup import _load_cached_files, get_filepath
    return _load_cached_files(get_filepath('files.configuration.units.units_json'), sync_files=sync_files)


def get_available_unit_categories(sync_files: bool = False) -> List[str]:
    ...
    """
    from ePy_docs.core.setup import _load_cached_files, get_filepath
    aliases_data = _load_cached_files(get_filepath('files.configuration.units.aliases_json'), sync_files=sync_files)
    return list(aliases_data["categories"].keys())


def get_aliases_for_category(category: str, sync_files: bool = False) -> Dict[str, str]:
    """Get aliases for a specific category from aliases file.
    
    Args:
        category: Category name to get aliases for
        sync_files: Whether to sync files before loading
        
    Returns:
        Dictionary of aliases for the category
    """
    from ePy_docs.core.setup import _load_cached_files, get_filepath
...
def get_unit_from_config(category: str, key: str, sync_files: bool = False) -> Optional[str]:
    """Get unit from configuration by category and key.
    
    Args:
        category: Unit category name
        key: Unit key within category
        sync_files: Whether to sync files before loading
        
    Returns:
        Unit string or None if not found
    """
    units_config = load_units_config(sync_files=sync_files)
...
def get_unit_precision(unit: str, sync_files: bool = False) -> int:
    ...
    if not unit or not units_config:
    ...
    # Direct lookup
    if unit in precision_config:
        return precision_config[unit]
    
    return 3  # Default precision


def get_prefix_data(sync_files: bool = False) -> Dict[str, Any]:
    """Get prefix data for unit conversions.
    
    Args:
        sync_files: Whether to sync files before loading
        
    Returns:
        Dictionary with prefix configuration
    """
    from ePy_docs.core.setup import _load_cached_files, get_filepath
    return _load_cached_files(get_filepath('files.configuration.units.prefix_json'), sync_files=sync_files)


def get_conversion_data(sync_files: bool = False) -> Dict[str, Any]:
    """Get conversion data for unit transformations.
    ...
    """
    from ePy_docs.core.setup import _load_cached_files, get_filepath
    return _load_cached_files(get_filepath('files.configuration.units.conversion_json'), sync_files=sync_files)


def get_format_data(sync_files: bool = False) -> Dict[str, Any]:
    """Get format data for unit display.
    ...
    """
    from ePy_docs.core.setup import _load_cached_files, get_filepath
    return _load_cached_files(get_filepath('files.configuration.units.format_json'), sync_files=sync_files)
